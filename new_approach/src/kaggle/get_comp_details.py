...
for index, competition in enumerate(competitions):
    ...
    # Only count as successfully scraped if we got at least one tab
    if tabs_successfully_scraped > 0:
        successfully_scraped += 1
        print(f"  ✅ Successfully scraped {tabs_successfully_scraped}/{len(TABS_TO_SCRAPE)} tabs for '{competition['name']}'")
    else:
        print(f"  ❌ Failed to scrape any tabs for '{competition['name']}'")

    if (index + 1) % 10 == 0 or (index + 1) == total_competitions:
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(competitions, f, indent=4, ensure_ascii=False)
        print(f"\n✅ Progress saved! Attempted {index + 1}/{total_competitions} competitions, successfully scraped {successfully_scraped}.\n")

print(f"\n🎉 Scraping complete! Attempted {total_competitions} competitions, successfully scraped {successfully_scraped}.")
